...
export default function contextWorkflowExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      let message = workflow.testsPassed
        ? `✅ Tests passed! (exit code: ${params.exitCode})`
        : `❌ Tests failed (exit code: ${params.exitCode})`;

      if (!workflow.testsPassed && params.output) {
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      let message =
        params.issues.length === 0
          ...
          : `📋 Review found ${params.issues.length} issue(s):\n${params.issues
              .map((issue, i) => `\n${i + 1}. ${issue}`)
              .join("")}`;
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    if (event.toolName !== "bash" || !event.input?.command || event.details?.exitCode === undefined) return;
